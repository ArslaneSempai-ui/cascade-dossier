...
test("la mesure publique refuse un registre vide, et dit une question absente telle quelle", () => {
  /* le registre RÉEL est vide tant que D1 n'est pas livré : le refus est le comportement
     du jour, et ce test le fige : un dossier sans jugement serait un vert vide */
  assert.throws(() => mesurePublique(REGLAGES), /registry is empty \(lot D1\)/);
});
